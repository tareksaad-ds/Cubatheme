...
export const signupAction = (user) => async (dispatch) => {
  ...
  const res = await Axios.post("http://localhost:5984/cuba_users/", user, {
    headers,
  });
  dispatch({
    type: SIGN_UP,
...
export const signinAction = (email, password, redirectData = null) => async (
  dispatch
) => {
  const headers = {
    Authorization: "Basic YWRtaW46YWRtaW4=",
  ...
  const res = await Axios.post(
    "http://localhost:5984/cuba_users/_find/",
    {
